...
# Add JS source
COPY ./translator/static /usr/src/app/translator/static
...
# Add app source
COPY ./translator /usr/src/app/translator
# Add built production JS assets
COPY --from=builder /usr/src/app/translator/static/dist /usr/src/app/translator/static/dist

ENV WEB_CONCURRENCY 1
WORKDIR /usr/src/app
EXPOSE 8080
USER nobody
CMD ["gunicorn", "-b", "0.0.0.0:8080", "--access-logfile", "-", "translator:app"]